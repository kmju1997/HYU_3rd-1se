...
struct proc* dequeue(struct Mlfq* q, struct proc* p)
{
    ...
    //if(ret->pid == p->pid)cprintf("pid  %d dequeue\n",ret->pid);
...
void printMLFQAll(){
    ...
    for(i=0;i<MAX_QUEUE_NUM;i++){
        ...
        for(j=0;j<MAX_QUEUE_SIZE;j++){
            if(qOfM[i].queue[j])
            cprintf(" pid %d /name %s /  state %d / cT %d / pass %d/ stride %d\n",qOfM[i].queue[j]->pid,qOfM[i].queue[j]->name,qOfM[i].queue[j]->state,qOfM[i].queue[j]->consumedTime, qOfM[i].queue[j]->pass, qOfM[i].queue[j]->stride);
