...
0000029a <mknod>:
SYSCALL(mknod)
 29a:	b8 12 00 00 00       	mov    $0x12,%eax
 29f:	cd 40                	int    $0x40
 2a1:	c3                   	ret    
...
000002b2 <link>:
SYSCALL(link)
 2b2:	b8 14 00 00 00       	mov    $0x14,%eax
 2b7:	cd 40                	int    $0x40
 2b9:	c3                   	ret    
...
putc(int fd, char c)
{
 ...
 3ba:	e8 b3 fe ff ff       	call   272 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 426:	e8 47 fe ff ff       	call   272 <write>
...
putc(int fd, char c)
{
 ...
 496:	e8 d7 fd ff ff       	call   272 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 4b3:	e8 ba fd ff ff       	call   272 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 5c0:	e8 ad fc ff ff       	call   272 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 6ca:	e8 13 fc ff ff       	call   2e2 <sbrk>
  if(p == (char*)-1)
