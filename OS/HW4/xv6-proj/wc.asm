...
putc(int fd, char c)
{
 ...
 51a:	e8 b3 fe ff ff       	call   3d2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
printf(int fd, char *fmt, ...)
{
 ...
 561:	e9 c2 00 00 00       	jmp    628 <printf+0xe8>
 566:	66 90                	xchg   %ax,%ax
    c = fmt[i] & 0xff;
    if(state == 0){
      if(c == '%'){
 568:	83 fa 25             	cmp    $0x25,%edx
 56b:	0f 84 bf 00 00 00    	je     630 <printf+0xf0>
#include "user.h"
...
{
  write(fd, &c, 1);
 586:	e8 47 fe ff ff       	call   3d2 <write>
 ...
 594:	0f 84 8e 00 00 00    	je     628 <printf+0xe8>
    c = fmt[i] & 0xff;
    if(state == 0){
 ...
 5a4:	75 e5                	jne    58b <printf+0x4b>
      if(c == 'd'){
 5a6:	83 fa 64             	cmp    $0x64,%edx
 5a9:	0f 84 31 01 00 00    	je     6e0 <printf+0x1a0>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
 5af:	25 f7 00 00 00       	and    $0xf7,%eax
 5b4:	83 f8 70             	cmp    $0x70,%eax
 5b7:	0f 84 83 00 00 00    	je     640 <printf+0x100>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
 5bd:	83 fa 73             	cmp    $0x73,%edx
 5c0:	0f 84 a2 00 00 00    	je     668 <printf+0x128>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
 ...
 5c6:	83 fa 63             	cmp    $0x63,%edx
 5c9:	0f 84 35 01 00 00    	je     704 <printf+0x1c4>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
 5cf:	83 fa 25             	cmp    $0x25,%edx
 5d2:	0f 84 e0 00 00 00    	je     6b8 <printf+0x178>
#include "user.h"
...
putc(int fd, char c)
{
 ...
 5ef:	89 55 d0             	mov    %edx,-0x30(%ebp)
 5f2:	c6 45 e6 25          	movb   $0x25,-0x1a(%ebp)
 5f6:	e8 d7 fd ff ff       	call   3d2 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 613:	e8 ba fd ff ff       	call   3d2 <write>
  int c, i, state;
  uint *ap;
  ...
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
 618:	0f b6 53 ff          	movzbl -0x1(%ebx),%edx
 61c:	84 d2                	test   %dl,%dl
 61e:	0f 85 76 ff ff ff    	jne    59a <printf+0x5a>
 ...
 62f:	c3                   	ret    
  ...
  for(i = 0; fmt[i]; i++){
    ...
    if(state == 0){
      if(c == '%'){
        state = '%';
 630:	bf 25 00 00 00       	mov    $0x25,%edi
 635:	e9 51 ff ff ff       	jmp    58b <printf+0x4b>
 ...
 655:	e8 46 fe ff ff       	call   4a0 <printint>
        ap++;
 65a:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
 65e:	e9 28 ff ff ff       	jmp    58b <printf+0x4b>
 663:	90                   	nop
...
putc(int fd, char c)
{
 ...
 6aa:	75 dc                	jne    688 <printf+0x148>
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
        putc(fd, c);
 ...
 6ae:	e9 d8 fe ff ff       	jmp    58b <printf+0x4b>
 6b3:	90                   	nop
...
putc(int fd, char c)
{
 ...
 6c9:	89 34 24             	mov    %esi,(%esp)
 6cc:	c6 45 e5 25          	movb   $0x25,-0x1b(%ebp)
 6d0:	e8 fd fc ff ff       	call   3d2 <write>
 6d5:	e9 b1 fe ff ff       	jmp    58b <printf+0x4b>
 ...
 6f6:	e8 a5 fd ff ff       	call   4a0 <printint>
        ap++;
 6fb:	83 45 d4 04          	addl   $0x4,-0x2c(%ebp)
 6ff:	e9 87 fe ff ff       	jmp    58b <printf+0x4b>
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
...
putc(int fd, char c)
{
 ...
 720:	e8 ad fc ff ff       	call   3d2 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
 ...
 729:	e9 5d fe ff ff       	jmp    58b <printf+0x4b>
...
morecore(uint nu)
{
 ...
 82a:	e8 13 fc ff ff       	call   442 <sbrk>
  if(p == (char*)-1)
