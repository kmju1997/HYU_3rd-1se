...
printint(int fd, int xx, int base, int sgn)
{
 ...
 459:	0f b6 92 7d 08 00 00 	movzbl 0x87d(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 49a:	e8 b3 fe ff ff       	call   352 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 506:	e8 47 fe ff ff       	call   352 <write>
...
putc(int fd, char c)
{
 ...
 576:	e8 d7 fd ff ff       	call   352 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 593:	e8 ba fd ff ff       	call   352 <write>
  int c, i, state;
  uint *ap;
 ...
 5ef:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 5f1:	b8 76 08 00 00       	mov    $0x876,%eax
...
putc(int fd, char c)
{
 ...
 6a0:	e8 ad fc ff ff       	call   352 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 7aa:	e8 13 fc ff ff       	call   3c2 <sbrk>
  if(p == (char*)-1)
