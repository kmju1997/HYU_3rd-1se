...
00000e02 <write>:
SYSCALL(write)
     e02:	b8 11 00 00 00       	mov    $0x11,%eax
     e07:	cd 40                	int    $0x40
     e09:	c3                   	ret    

00000e0a <close>:
SYSCALL(close)
     e0a:	b8 16 00 00 00       	mov    $0x16,%eax
     e0f:	cd 40                	int    $0x40
     e11:	c3                   	ret    
...
00000e2a <mknod>:
SYSCALL(mknod)
     e2a:	b8 12 00 00 00       	mov    $0x12,%eax
     e2f:	cd 40                	int    $0x40
     e31:	c3                   	ret    
...
00000e42 <link>:
SYSCALL(link)
     e42:	b8 14 00 00 00       	mov    $0x14,%eax
     e47:	cd 40                	int    $0x40
     e49:	c3                   	ret    
...
00000e82 <uptime>:
SYSCALL(uptime)
     e82:	b8 0f 00 00 00       	mov    $0xf,%eax
     e87:	cd 40                	int    $0x40
     e89:	c3                   	ret    
...
00000ea2 <set_cpu_share>:
SYSCALL(set_cpu_share)
     ea2:	b8 1a 00 00 00       	mov    $0x1a,%eax
     ea7:	cd 40                	int    $0x40
     ea9:	c3                   	ret    

00000eaa <thread_create>:
SYSCALL(thread_create)
     eaa:	b8 1b 00 00 00       	mov    $0x1b,%eax
     eaf:	cd 40                	int    $0x40
     eb1:	c3                   	ret    

00000eb2 <thread_exit>:
SYSCALL(thread_exit)
     eb2:	b8 1c 00 00 00       	mov    $0x1c,%eax
     eb7:	cd 40                	int    $0x40
     eb9:	c3                   	ret    

00000eba <thread_join>:
SYSCALL(thread_join)
     eba:	b8 1d 00 00 00       	mov    $0x1d,%eax
     ebf:	cd 40                	int    $0x40
     ec1:	c3                   	ret    
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     f4a:	e8 b3 fe ff ff       	call   e02 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
     fb6:	e8 47 fe ff ff       	call   e02 <write>
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    1026:	e8 d7 fd ff ff       	call   e02 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
    1043:	e8 ba fd ff ff       	call   e02 <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    1150:	e8 ad fc ff ff       	call   e02 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    125a:	e8 13 fc ff ff       	call   e72 <sbrk>
  if(p == (char*)-1)
