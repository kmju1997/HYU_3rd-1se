...
main(int argc, char *argv[])
{
  ...
  3f:	e8 ec 03 00 00       	call   430 <printf>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3c9:	0f b6 92 a6 07 00 00 	movzbl 0x7a6(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 40a:	e8 b3 fe ff ff       	call   2c2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 476:	e8 47 fe ff ff       	call   2c2 <write>
...
putc(int fd, char c)
{
 ...
 4e6:	e8 d7 fd ff ff       	call   2c2 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 503:	e8 ba fd ff ff       	call   2c2 <write>
  int c, i, state;
  uint *ap;
 ...
 55f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 561:	b8 9f 07 00 00       	mov    $0x79f,%eax
...
putc(int fd, char c)
{
 ...
 610:	e8 ad fc ff ff       	call   2c2 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 71a:	e8 13 fc ff ff       	call   332 <sbrk>
  if(p == (char*)-1)
