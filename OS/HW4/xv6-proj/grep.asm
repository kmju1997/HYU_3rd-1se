...
printint(int fd, int xx, int base, int sgn)
{
 ...
 659:	0f b6 92 65 0a 00 00 	movzbl 0xa65(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 69a:	e8 b3 fe ff ff       	call   552 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 706:	e8 47 fe ff ff       	call   552 <write>
...
putc(int fd, char c)
{
 ...
 776:	e8 d7 fd ff ff       	call   552 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 793:	e8 ba fd ff ff       	call   552 <write>
  int c, i, state;
  uint *ap;
 ...
 7ef:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 7f1:	b8 5e 0a 00 00       	mov    $0xa5e,%eax
...
putc(int fd, char c)
{
 ...
 8a0:	e8 ad fc ff ff       	call   552 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 9aa:	e8 13 fc ff ff       	call   5c2 <sbrk>
  if(p == (char*)-1)
