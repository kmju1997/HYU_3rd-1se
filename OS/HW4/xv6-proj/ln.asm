...
000002ba <close>:
SYSCALL(close)
 2ba:	b8 16 00 00 00       	mov    $0x16,%eax
 2bf:	cd 40                	int    $0x40
 2c1:	c3                   	ret    
...
putc(int fd, char c)
{
 ...
 3fa:	e8 b3 fe ff ff       	call   2b2 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 466:	e8 47 fe ff ff       	call   2b2 <write>
...
putc(int fd, char c)
{
 ...
 4d6:	e8 d7 fd ff ff       	call   2b2 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 4f3:	e8 ba fd ff ff       	call   2b2 <write>
  int c, i, state;
  uint *ap;
 ...
 54f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 551:	b8 ad 07 00 00       	mov    $0x7ad,%eax
...
putc(int fd, char c)
{
 ...
 600:	e8 ad fc ff ff       	call   2b2 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 70a:	e8 13 fc ff ff       	call   322 <sbrk>
  if(p == (char*)-1)
