...
0000038a <close>:
SYSCALL(close)
 38a:	b8 16 00 00 00       	mov    $0x16,%eax
 38f:	cd 40                	int    $0x40
 391:	c3                   	ret    
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 489:	0f b6 92 80 08 00 00 	movzbl 0x880(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 4ca:	e8 b3 fe ff ff       	call   382 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 536:	e8 47 fe ff ff       	call   382 <write>
...
putc(int fd, char c)
{
 ...
 5a6:	e8 d7 fd ff ff       	call   382 <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 5c3:	e8 ba fd ff ff       	call   382 <write>
  int c, i, state;
  uint *ap;
 ...
 61f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
 621:	b8 79 08 00 00       	mov    $0x879,%eax
...
putc(int fd, char c)
{
 ...
 6d0:	e8 ad fc ff ff       	call   382 <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 7da:	e8 13 fc ff ff       	call   3f2 <sbrk>
  if(p == (char*)-1)
