...
printPtable(void)
{
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        if(p->pid != 0)
            cprintf(" pid %d / name %s /state %d / cT %d / stride %d / pass %d / tid %d\n",p->pid,p->name, p->state,p->consumedTime, p->stride, p->pass, p->tid);
...
thread_join(thread_t thread, void **retval){
    ...
    for(;;){
        ...
        // No point waiting if we don't have any children.
        if(!havekids || proc->killed){
        cprintf(" [Error]No child join error\n");
            release(&ptable.lock);
            return -1;
        }
        ...
        // Wait for children to exit.  (See wakeup1 call in proc_exit.)
        sleep((void*)proc->pid, &ptable.lock);  //DOC: wait-sleep
